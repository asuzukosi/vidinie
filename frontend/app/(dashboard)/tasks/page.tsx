...
const stageLabels: Record<
  TaskStage,
  { label: string; step: number; color: string }
> = {
...
function StageIndicator({ stage }: { stage: TaskStage }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium text-white ${info.color}`}
    >
...
function CreateTaskModal({
  ...
  const handleCreateTask = () => {
    ...
    } else if (activeTab === "url" && pastedContent.trim()) {
      taskName = pastedContent.substring(0, 50).trim() || "Upload URL";
    }
  ...
  const canCreate =
    ...
    (activeTab === "url" && pastedContent.trim());
        ...
        {activeTab === "upload" ? (
          ...
          <div>
            <Input
              ...
              placeholder="Paste your URL content here..."
