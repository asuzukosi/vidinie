...
import { VideoPipelineProcessingStage, VideoPipelineStage, VideoPipelineStatus, GenerateVideoPipelineRequest, CreateVideoOutlineRequest, VideoPipeline } from "@/lib/sdk/types";
...
// helper function to get status from flat fields
const getStageStatus = (videoPipeline: VideoPipeline, stage: VideoPipelineStage): VideoPipelineStatus | null => {
  switch (stage) {
    case VideoPipelineStage.INITIALIZED:
      return videoPipeline.initialized_status || null;
    case VideoPipelineStage.DOCUMENT_PROCESSING:
      return videoPipeline.document_processing_status || null;
    case VideoPipelineStage.IMAGE_PROCESSING:
      return videoPipeline.image_processing_status || null;
    case VideoPipelineStage.CONTENT_ANALYSIS:
      return videoPipeline.content_analysis_status || null;
    case VideoPipelineStage.SCRIPT_GENERATION:
      return videoPipeline.script_generation_status || null;
    case VideoPipelineStage.VIDEO_GENERATION:
      return videoPipeline.video_generation_status || null;
    default:
      return null;
  }
};
...
export function VideoPipelineStagesManager({ 
  ...
  const anyTaskInProgress = useMemo(() => {
    return (
      videoPipeline.initialized_status === VideoPipelineStatus.IN_PROGRESS ||
      videoPipeline.document_processing_status === VideoPipelineStatus.IN_PROGRESS ||
      videoPipeline.image_processing_status === VideoPipelineStatus.IN_PROGRESS ||
      videoPipeline.content_analysis_status === VideoPipelineStatus.IN_PROGRESS ||
      videoPipeline.script_generation_status === VideoPipelineStatus.IN_PROGRESS ||
      videoPipeline.video_generation_status === VideoPipelineStatus.IN_PROGRESS
    );
  }, [videoPipeline]);
  ...
  const updateCurrentSteps = () => {
    const updatedSteps = pipelineProcessingStages.map((stage) => {
      const stageStatus = getStageStatus(videoPipeline, stage.id as VideoPipelineStage);
  ...
  const getStageStatusFromPipeline = (stepId: VideoPipelineStage | string): VideoPipelineStatus | null => {
    return getStageStatus(videoPipeline, stepId as VideoPipelineStage);
  }

  const isStageProcessing = (stepId: VideoPipelineStage | string): boolean => {
    const status = getStageStatusFromPipeline(stepId);
